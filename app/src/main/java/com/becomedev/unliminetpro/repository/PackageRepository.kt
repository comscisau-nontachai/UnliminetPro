...
class PackageRepository(private val api: ApiService){
    ...
    suspend fun getPackageAis(): UiState<PromotionModel> {
        return try {
            val response = api.getAisPackage()
            response.body()?.let {
                UiState.Success(it)
            } ?: UiState.Error("Empty response body")
    ...
    suspend fun getPackageDtac(): UiState<PromotionModel> {
        return try {
            val response = api.getDtacPackage()
            response.body()?.let {
                UiState.Success(it)
            } ?: UiState.Error("Empty response body")
